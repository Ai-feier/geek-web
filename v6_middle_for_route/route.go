...
func (r *router) findRoute(method string, path string) (*matchInfo, bool) {
	...
	mi := &matchInfo{}
	for _, s := range segs {
		var matchParam bool  // 标记是否为参数匹配
		root, matchParam, ok = root.childOf(s)
		if !ok {  // 没找到
			return nil, false
		}
		if matchParam {
			mi.addValue(path[1:], s)
		}
	}
	mi.n = root
...
func (r *router) findMdls(root *node, segs []string) (res []Middleware) {
	// 层序遍历
	// 先添加通配符 Middleware
	// 然后 参数匹配的 Middleware
	// 最后 静态 Middleware
	...
	for i := 0; i < len(segs); i++ {
		var children []*node
		for _, cur := range q {
			if len(cur.matchedMdls) > 0 {
				res = append(res, cur.matchedMdls...)
			}
			children = append(children, cur.childrenOf(segs[i])...)
		}
		q = children
	}
	// q 队列中还有最后一段的节点
...
// 第三个返回值 bool 代表是否命中
func (n *node) childOf(s string) (*node, bool, bool) {
	if n.children == nil {
		if n.paramChild != nil {
